...
public class CategoryRepository : ICategoryRepository
{
	...
	{
		var category = await _rosaFiestaContext.Categories.Include(x => x.Products).ThenInclude(x => x.Option).ThenInclude(x => x.Image).FirstOrDefaultAsync(x => x.Id == categoryId);
		if (category == null)
