...
internal sealed class ReviewRepository: IReviewRepository
{
    ...
    public async Task<IEnumerable<ReviewEntity>> GetAllAsync(CancellationToken cancellationToken = default) =>
        await _context.Reviews.Include(x => x.Product).Include(x => x.UserEntity).ToListAsync(cancellationToken);

    public async Task<ReviewEntity> GetByIdAsync(Guid reviewId, CancellationToken cancellationToken = default)
    {
        ReviewEntity? review = await _context.Reviews.Include(x => x.Product).Include(x => x.UserEntity)
    ...
    public async Task AlredyExistAsync(string productCode, int? optionId, string userId, CancellationToken cancellationToken = default)
    {
        ReviewEntity? review = await _context.Reviews.FirstOrDefaultAsync(x => x.ProductCode == productCode && x.UserReviewerId == userId && (optionId == null || x.OptionId == optionId), cancellationToken);
        if (review != null)
