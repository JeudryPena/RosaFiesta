...
export class ModalDiscountComponent implements OnInit {
    ...
    async onRead() {
        ...
        const form = this.fb.group({
            value: [{value: [response.value], disabled: this.read}],
            date: [{value: [this.date], disabled: this.read}],
            start: [{value: [this.minDate], disabled: this.read}],
            end: [{value: [this.maxDate], disabled: this.read}],
            createdAt: [{value: [this.datePipe.transform(response.createdAt, 'dd-MMM-yyyy h:mm:ss a')], disabled: this.read}],
